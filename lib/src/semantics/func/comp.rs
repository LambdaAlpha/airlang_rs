...
impl FreeComposite {
    pub(super) fn call(
        ...
        Eval.ctx_call(cfg, new_ctx, body)
    }
}

impl DynComposite {
    // todo design support ignore context or input
    pub(super) fn call(&self, cfg: &mut Cfg, ctx: &mut Val, const_: bool, input: Val) -> Val {
        ...
        let output = Eval.ctx_call(cfg, new_ctx, self.body.clone());
        let mut new_ctx =
