...
where Fn: ConstStaticFn<Val, Val, Val>
{
    fn const_static_call(&self, ctx: ConstRef<Val>, input: Symbol) -> Val {
        ...
        match prefix {
            SYMBOL_LITERAL_CHAR => Val::Symbol(s),
            SYMBOL_REF_CHAR => {
                let Some(val) = get_dyn_ref(ctx.unwrap(), s) else {
                    return Val::default();
                };
                val.clone()
            }
            SYMBOL_EVAL_CHAR => {
                let ctx = ctx.unwrap();
                let Some(val) = get_dyn_ref(ctx, s) else {
                    return Val::default();
                };
                let val = val.clone();
...
fn get_dyn_ref(ctx: &mut Val, name: Symbol) -> Option<DynRef<'_, Val>> {
    match ctx {
        ...
        match self.func.const_static_call(c.reborrow(), task.func) {
            Val::Func(func) => match task.action {
                Action::Call => {
                    ...
                    let Some(c) = eval_const_ctx(c, task.ctx) else {
                        return Val::default();
                    };
            ...
            Val::Symbol(func) => with_lock(c.unwrap(), func, |c, func, _| match task.action {
                Action::Call => {
                    ...
                    let Some(c) = eval_const_ctx(ConstRef::new(c), task.ctx) else {
        ...
        match self.func.mut_static_call(c, task.func) {
            Val::Func(func) => match task.action {
                Action::Call => {
                    ...
                    let Some(c) = eval_mut_ctx(c, task.ctx) else {
                        return Val::default();
                    };
    ...
    fn const_static_call(&self, ctx: ConstRef<Val>, task: TaskVal) -> Val {
        ...
        match task.func {
            ...
            Val::Symbol(func) => with_lock(ctx.unwrap(), func, |ctx, func, _| {
                let Some(ctx) = eval_const_ctx(ConstRef::new(ctx), task.ctx) else {
                    return (func, Val::default());
                };
                match task.action {
                    Action::Call => {
