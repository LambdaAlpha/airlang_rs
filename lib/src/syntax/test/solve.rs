...
pub(super) fn expected() -> Vec<Repr> {
    vec![
        ...
        infix_call(solve(key("a"), unit()), key("b"), key("c")),
        pair(solve(key("a"), unit()), key("c")),
        infix_call(solve(key("a"), unit()), key("b"), key("c")),
        pair(solve(key("a"), unit()), key("c")),
        infix_call(solve(key("a"), unit()), key("b"), key("c")),
        pair(solve(key("a"), unit()), key("c")),
        ...
        infix_call(key("a"), key("b"), solve(key("c"), unit())),
        pair(key("a"), solve(key("c"), unit())),
        infix_call(key("a"), key("b"), solve(key("c"), unit())),
        pair(key("a"), solve(key("c"), unit())),
        infix_call(key("a"), key("b"), solve(key("c"), unit())),
        pair(key("a"), solve(key("c"), unit())),
