...
pub struct CallMode {
    ...
    pub some: Option<CallMapMode>,
}

pub type CallMapMode = Map<Val, CtxInput<Option<Mode>, Option<Mode>>>;
...
impl FreeStaticFn<CallVal, Val> for CallMode {
    fn free_static_call(&self, input: CallVal) -> Val {
        match &self.some {
            Some(some) => CallForm { func: &self.func, ctx: &self.ctx, input: &self.input, some }
                .free_static_call(input),
            None => CallEval { func: &self.func, ctx: &self.ctx, input: &self.input }
                .free_static_call(input),
        }
    }
}
