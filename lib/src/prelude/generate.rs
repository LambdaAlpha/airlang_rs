...
#[derive(Clone)]
pub(crate) struct GeneratePrelude {
    pub(crate) new: Named<FuncVal>,
    pub(crate) get_func: Named<FuncVal>,
    pub(crate) set_func: Named<FuncVal>,
}

impl Default for GeneratePrelude {
    fn default() -> Self {
        GeneratePrelude { new: new(), get_func: get_func(), set_func: set_func() }
    }
}

impl Prelude for GeneratePrelude {
...
fn fn_new(input: Val) -> Val {
    Val::Generate(Generate::new(input).into())
}

fn get_func() -> Named<FuncVal> {
    let id = "generate.function";
    let f = fn_get_func;
...
fn fn_get_func(ctx: ConstFnCtx, input: Val) -> Val {
    ...
    DefaultCtx::with_dyn(ctx, pair.first, |ref_or_val| match ref_or_val {
        Either::This(val) => match val.as_const() {
            Val::Generate(generate) => generate.func.clone(),
            _ => Val::default(),
        },
        Either::That(val) => match val {
            Val::Generate(generate) => Generate::from(generate).func,
            _ => Val::default(),
        },
    })
}
...
fn fn_set_func(ctx: MutFnCtx, input: Val) -> Val {
    ...
    DefaultCtx::with_dyn(ctx, name, |ref_or_val| match ref_or_val {
        Either::This(mut generate) => {
            let Some(Val::Generate(generate)) = generate.as_mut() else {
                return Val::default();
            };
            swap(&mut generate.func, &mut val);
            val
        }
