...
`配置`是执行过程中的全局信息环境，通过仅追加与局域覆盖等机制，兼顾了灵活性和可预测性。可通过 `import` 函数导入配置项，或通过 `export` 函数导出配置项，亦可通过 `with` 函数局域覆盖配置项。我们将基于配置机制实现模块管理、任务管理、错误管理和测试框架等特性，并在初始配置中提供原生函数和标准库。
